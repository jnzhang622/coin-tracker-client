...
=======
  trackCoin = () => {
    fetch("http://localhost:3000/api/v1/user_coins", {
      ...
      .then((data) => {
        console.log(data);
        Array.isArray(data) ? this.props.setUser(data) : alert(data.message);
      });
  };
...
=======
      volume = this.props.coin[0][`${this.state.priceRenderChanges}`].volume;
      price_change = this.props.coin[0][`${this.state.priceRenderChanges}`]
        .price_change;
      price_change_pct = this.props.coin[0][`${this.state.priceRenderChanges}`]
        .price_change_pct;
      volume_change = this.props.coin[0][`${this.state.priceRenderChanges}`]
        .volume_change;
      volume_change_pct = this.props.coin[0][`${this.state.priceRenderChanges}`]
        .volume_change_pct;
      market_cap_change = this.props.coin[0][`${this.state.priceRenderChanges}`]
        .market_cap_change;
      market_cap_change_pct = this.props.coin[0][
        `${this.state.priceRenderChanges}`
      ].market_cap_change_pct;
>>>>>>> 11d10ec6e7d00ace43c64f405587973631582bc6
    }
...
=======
              <div>
                <button className="card_button">Post Comment</button>
                <button onClick={this.trackCoin} className="card_button">
                  Add to Watch
                </button>
              </div>
            </div>
>>>>>>> 11d10ec6e7d00ace43c64f405587973631582bc6
            ...
            {this.props.coin[0][`1d`] ? (
              <div className="card__content">
                ...
                <div>
                  ...
                  <a className="showPage__info">
                    Market Cap Change: {market_cap_change}
                  </a>
                  <a className="showPage__info">
                    Market Cap Change %: {market_cap_change_pct}%
                  </a>
                </div>
              </div>
            ) : null}
          ...
          <div>
            <CoinComments
              currentUser={this.state.user_name}
              symbol={this.state.symbol}
            />
          </div>
